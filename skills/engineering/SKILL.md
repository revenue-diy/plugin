...
*IF the task is SIMPLE (one small artefact, an explicit "just do it") OR `simple fix`* → FLAG `design clear`, settle the inputs in one confirmation line, SKIP to `§ STEP 2: PLAN`.
...
1. **Analyse targets** – read the target files or artefacts in full.
...
*IF `Brief review` is selected* → first dispatch `revenue-diy:reviewer` (sonnet) – **`Reviewer (sonnet): brief vs decisions`** – `{target:[brief.md § PLAN + § WHAT-NOT-TO-DO], rubric:[every § AGENDA row LOCKED with a D-ID and no detailed row locked before its structural parent; every D-ID mapped to a work item or an explicit exclusion; no omission, duplication or contradiction], references:[brief.md § AGENDA + § DECISIONS], review:[<scratch>/review_brief.md]}`. FIX findings before asking.

1. **Print the summary block** – **Plan overview** (items in order, landing per item) · **Autonomy boundaries** · **Review and test approach**. 2. **Link the brief.** 3. **Ask via `AskUserQuestion`:** *Plan approval: do you approve the plan as briefed?* – **Approve** · **Adjust**. 4. **On Approve** – READ `workflows/connector.md § LOG` and follow it for `brief.md` and every research report not yet logged.
...
✅ Brief review passed; approved ON the presented plan (`simple fix`: work log only)?
...
*IF the session states a one-hour prompt-cache TTL AND agents are running* → *IF a session-timer tool is in your tools* → a 55-minute timer doing a silent status check, re-armed while agents remain, deleted when they land.
...
**Route findings** (P1 blocks · P2 should fix · P3 nice): impeaches a locked input → STOP and report; P1/P2 → fix, re-run this STEP; P3 → trivial fix or report at WRAP-UP. Log every review in brief `§ REVIEW RESULTS` with `F<n>` IDs.
...
2. **Log the final report and sweep** – write the `§ REPORT TO USER` text to `<scratch>/report_final.md`, READ `workflows/connector.md § LOG` and follow it for that file and for every other report in `<scratch>` still without a `logged:` line. THEN VERIFY: every `*.md` report in `<scratch>` carries a `logged: <file> → <url>` line in brief `§ PROGRESS TRACKING`; list any that do not and post them. *IF `worklog: required`* → closing with one missing is a gate failure.
...
✅ Documentation final, test plan persisted where required; brief `completed`; work log closed, every `*.md` report in `<scratch>` carrying a `logged:` line (*IF `worklog: required`*)?
