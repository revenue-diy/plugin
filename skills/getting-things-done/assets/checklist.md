...
7. **Session opened with STEP 0** – `brief.md` was created first (with `steps`, `progress`, `connector`, `worklog` and `log` keys), then `connector § DETECT` ran: with the connector present the opening `get_context` carried `skill` and `log: "start"`; absent, the single fixed line was printed – before any prose either way. Evidence: quote the brief header and the call (or the line), in the order they happened.
